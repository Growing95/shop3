...
function chkdelete_cart() {
	location.href="/MyController?cmd=chkdelete"
}
</script>
</head>
<body>
	...
	<table>
		<caption>
			<h2>:: 장바구니 내용 ::</h2><button onclick="chkdelete_cart()" style="float: left;">선택항목 삭제</button>
		</caption>
		<thead>
			<tr bgcolor="#dedede">
		...
		<tbody>
			<c:choose>
				<c:when test="${empty clist }">
					<tr>
						<td colspan="7"><h3> 장바구니가 비었습니다.</h3> </td>
					</tr>
				</c:when>
				<c:otherwise>
					<c:forEach var="k" items="${clist}">
						<tr>
							...
							<input type="checkbox" name="c_idx" value="c_idx">
							</form>
							</td>
							<td>${k.p_num }</td>
							<td>${k.p_name }</td>
							...
							<td>${k.p_saleprice*k.amount }원</td>
							<td>
								<form method="post">
									<input type="hidden" name="id" value="${k.id }">
									<input type="hidden" name="p_num" value="${k.p_num}">
